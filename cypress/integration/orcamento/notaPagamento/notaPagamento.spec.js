...
class notaPagamento {
  geraPagamento() {
    ...
    it("Nota de Pagamento - Valida Modal Pesquisa e insere numero da Liquidação", () => {
      ...
      cy.get(
        'button[nat="cadastroNotaPagamentoGridabrirTelaDeCadastro"]'
      ).click();
    });

    it("Nota de Pagamento - Informa itens do pagamento com valor maior que o saldo", () => {
      //Pesquisa Numero da conta
      cy.get('button[nat="cadastroNotaPagamentoItemNrContaPesquisa"]', {
        timeout: 2000,
      }).click();
      cy.get(
        'div[nat="pesquisaContaBancariaGrid"]>div>div>div>div[class="ui-grid-canvas"]>div>div',
        { timeout: 60000 }
      );

      //Pesquisa Gestão da conta
      cy.autoComplete(
        'input[nat="pesquisaContaBancariaGestaoDescricao"]',
        "FUNDO MUNICIPAL DE PROTEÇÃO E DEFESA DO CONSUMIDOR-FUNDECON"
      );
      cy.get('button[nat="Pesquisar"]').click();
      cy.get(
        'div[nat="pesquisaContaBancariaGrid"]>div>div>div>div[class="ui-grid-canvas"]>div>div',
      ...
      //Valor Bruto
      cy.get('input[nat="cadastroNotaPagamentoItemValorBruto"]')
        .dblclick()
        .type("10,00", { Delay: 10 });

      //Forma de Pagamento
      cy.autoComplete(
        'input[nat="cadastroNotaPagamentoItemFormaPagamentoDescricao"]',
        "ORDEM DE PAGAMENTO"
      );

      //Fonte Detalhamento
      cy.autoComplete(
        'input[nat="cadastroNotaPagamentoItemFonteDetalhamento"]',
        "100000 - RECURSO PROPRIO"
      );
      ...
      cy.get(".modal-body", { timeout: 2000 })
        .should("be.visible")
        .contains(/Erro ao efetuar pagamento de empenho/i);
    ...
    it("Nota de Pagamento - Insere retenção após anulaçao da liquidação", () => {
      ...
      //Clica no botão ADICIONAR
      cy.get(
        'button[nat="cadastroNotaPagamentoRetencaoGridabrirTelaDeCadastro"]'
      ).click();

      //Cadastro de retenções de pagamento
      cy.autoComplete(
        'input[nat="cadastroRetencaoPagamentoTipoRetencaoDescricao"]',
        "INSS  TERCEIROS"
      );

      //Informa Valor da Retenção
      cy.get('input[nat="cadastroRetencaoPagamentoValor"]')
        .click()
        .type("1,00", { Delay: 10 });
