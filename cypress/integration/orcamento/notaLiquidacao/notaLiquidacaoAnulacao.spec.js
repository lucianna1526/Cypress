    ...
    anulaLiquidacao() {
       it('ORÇAMENTO: ANL - Anulação de liquidação', () => {
        ...
        it('Nota de Liquidação / Anulação de liquidação - Valida Modal Pesquisa e insere numero de Ficha', () => {
            ...
            //Pesquisa Ficha
            cy.get('button[nat="anulacaoLiquidacaoFichaPesquisa"]').click().wait(4000);
        ...
        it('Nota de Liquidação / Anulação de Liquidação com valor acima do saldo', () => {
            ...
            cy.get('input[nat="cadastroAnulacaoLiquidacaoData"]').dblclick().type(formatedDate2PtBR()).wait(2000);

            //Informa o valor da Anulação da liquidação
            cy.get('input[nat="cadastroAnulacaoLiquidacaoValorLiquido"]').dblclick().type('10,00').wait(1000);
            
            //Informa numero de processo
            cy.get('input[nat="cadastroAnulacaoLiquidacaoProcesso"]').click().type('2022014911').wait(1000);           

            //Informa o historico da anulação
            cy.get('textarea[nat="cadastroAnulacaoLiquidacaoHistorico"]').click().type('ANULAÇAO DE LIQUIDAÇAO REALIZADO PELA AUTOMAÇAO', {Delay: 10}).wait(1000);
            
            //CLica no botão ADICIONAR E SAIR
            cy.get('button[nat="cadastroAnulacaoLiquidacaoCrudSalvarFechar"]').click();
            
            //valida se a anulação de liquidação foi salva com sucesso            
            cy.get('.modal-body', {timeout:2000}).should('be.visible').contains(/Valor da liquidação/i);   
        ...
        it('Nota de Liquidação / Anulação de Liquidação com valor dentro do saldo', () => {            
            ...
            //Informa o valor da Anulação da liquidação
            cy.get('input[nat="cadastroAnulacaoLiquidacaoValorLiquido"]').dblclick().type('1,00').wait(1000);
