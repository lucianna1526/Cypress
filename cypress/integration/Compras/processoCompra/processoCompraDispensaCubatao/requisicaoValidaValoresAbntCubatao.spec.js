class requisicaoValidaValoresAbntCubatao {
  validaCamposAbntCubataoReq() {
    ...
    it("Valida campos obrigátorios", () => {
      ...
      cy.get('[nat="cadastroRequisicaoComprasCrudLimpar"]').click().wait(5000);
    ...
    it("Validação Arredondamento SOMA Valor Autorizado", () => {
      ...
      cy.get("@grid").find(".ui-grid-footer-cell").as("coluna");
    ...
    it("Validação Arredondamento SOMA Valor TOTAL", () => {
      ...
      cy.get("@coluna").eq(8).contains("10,62").should("length", 1);
    });
    ...
    //div[nat="cadastroRequisicaoComprasReservasGrid"]>div>div>ui-grid-footer>div>div>div>div>div>div
    it("valida somatoria grid aba Reservas", () => {
      cy.get(
        'div[nat="cadastroRequisicaoComprasReservasGrid"]>div>div>ui-grid-footer>div>div>div>div>div>div'
      ).as("footer");
      ...
      cy.get("@footer").eq(5).contains("10,62").should("length", 1);
    });
    it("valida somatoria grid aba Reservas", () => {
