class notaFiscalAvulsa {
  geraNotaFiscalAvulsa() {
    it("NFE ADMINISTRAÇAO: Emissao da Fiscal Eletronica Avulsa", () => {
      cy.intercept("**/rest/cidadeController/consultarCodigoDescricao").as(
    ...
    it("Insere dados da nota fiscal avulsa", () => {
      ...
      cy.intercept(
        "**/rest/servicoCnaeController/getServicosDoCnae?codigo_cnae=4751201"
      ...
      cy.intercept("**/rest/nfeInssCapaController/getValorInss?valorNota=0").as(
        "nfeInssCapaController"
      );
      cy.intercept(
        "**/rest/nfeImpostoRendaCapaController/getValorIrrf?valorNota=0"
      ).as("nfeImpostoRendaCapaController");
      cy.clicaSpan("Serviço", "0", '[role="button"]');

      cy.digitaSpan(
        "Serviço",
        "31.01 - Serviços técnicos em edificações, eletrônica, eletrotécnica, mecânica, telecomunicações e congêneres.{enter}"
      );
      cy.wait("@nfeInssCapaController");
      cy.wait("@nfeImpostoRendaCapaController").wait(1000);
      cy.digitaSpan(
        "Descrição do serviço",
        "Teste de serviço gerado pelo automação",
        "0",
        "textarea"
      );
    ...
    it("Valida IMPRIMI NOTA FISCAL / GERAR DUAM", () => {
      ...
      cy.get(".pd-tela-padrao-titulo-popup", { timeout: 10000 }).contains(
        "Conferencia de Nota Fiscal Eletrônica Avulsa"
      );
