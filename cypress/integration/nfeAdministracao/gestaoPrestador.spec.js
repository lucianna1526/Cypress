...
class gestaoPrestador {
    replicarNfe() {
        ...
        it('Seleciona item na grid para replicar', () => {
            ...
            if (cy.get(".pd-grid-div").contains('100,00') || cy.get(".pd-grid-nenhum-registro").contains('Nenhuma nota fiscal para essa inscrição').should('be.visible')) {
            //Clica no Botao Anterior para voltar ao mes anterior caso nao encontre nada na grid na primeira pesquisa do mes
            cy.get('.pd-input-mes > .form-group > .input-group > .input-group-btn > [nat="BotaoAnterior"]').click();
